...
public $helpers = array('Html', 'Tinymce', 'Form', 'Javascript');
 ...
 */
	public function input($fieldName, $options = array(), $zInput=array()) {
		...
		//print_r($schemas_table);
		if (!empty($schemas_table['comment'])) {
			$options['between'] = '<small>'. $schemas_table['comment'] .'</small><br>';
			// $options['between'] = '<span class="between">'. $schemas_table['comment'] .'</span><br>';
		...
		switch ($type) {
			case 'hidden':
				...
				if(strPos($fieldName, '_th_hidden')>0 && $this->request->params['action']=='edit' && $input['value']!='') {
					...
					*/
					$input = '<div id="'.$nomeCampo.'"><small><a href="javascript:void(0);" class="deleteImagem"><img src="'.$this->webroot.$input['value'].'" alt="'.$nomeCampo.'" /><br /></a></small><br /><br /></div>';

					$input .= '	<script>
									$(".deleteImagem").click(function(){
										var imagem = $(this).children("img").attr("alt");
										//alert("'. $nomeCampo .'____"+imagem);

										$(".imagensToDelete").val( $("#imagensToDelete").val() + imagem +",");
										//$("#'.$nomeCampo.'").hide();
										$("#"+imagem).hide();
									});
								</script>';

					$input .= '<input type="hidden" name="data['.$modelKey.'][deleteimagens]" id="imagensToDelete" class="imagensToDelete" />';
				}
			...
			case 'file':
				...
				$dimensoes_file = 'Dimensões: '.$fileWidth.'x '.$fileHeight.'px <strong>(A imagem será redimencionada proporcionalmente à este limite)</strong>';
				if ($fileWidth == 0 && $fileHeight == 0):
					$dimensoes_file = '';
				endif;

				// >>> mostrar arquivo no scaffold

				$arquivo = '';
				if(!empty($this->request->data[$modelKey][$fieldName])):
					$file = $this->request->data[$modelKey][$fieldName];
					// print_r($file);
					if ($file['error'] != 4){
						$array = explode('/', $file);
						if(!empty($array[3])){
							$arquivo = '<div id="tag_'.$fieldNameJS.'" class="arquivo-file-btn"><p><strong>'.$array[3].'</strong><p></div>';
				...
				$fileDimensions = '<div><small>'.$dimensoes_file.'<br />Formatos permitidos: '.$fileExtensions.'</small></div>';
				...
				$input = $this->file($fieldName, $options) .$arquivo. $fileDimensions;
				// Função para verificar se o nome do arquivo é valido

				$input .= '<div id="'.$fieldName.'"><small><a href="javascript:void(0);" rel-id="'.$fieldNameJS.'" rel-name="'.$fieldName.'" class="excluir"><input type="hidden" value="'.@$this->request->data[$modelKey][$fieldName].' "><br />[excluir]</a></small><br /><br /></div>';

				$input .= '	<script>
								$(".excluir").click(function(){
									var arquivo = $(this).children("input").attr("value");
									var id_hidden = $(this).attr("rel-id");
									var field_name = $(this).attr("rel-name");

									$(this).find("input[type=hidden]").val(" ");


									$(this).parent().parent().parent().fadeOut();
									$("#"+id_hidden).attr("value", " ").attr("type", "hidden");

									var valor_campo_excluir = $(".arquivoExcluir").val();

									var campos_para_excluir = valor_campo_excluir;
									if (valor_campo_excluir.indexOf(field_name) == -1) {
										var separacao = "";
										// alert(valor_campo_excluir.length);

										if (valor_campo_excluir.length > 0) {
											separacao = ",";
										}
										campos_para_excluir = campos_para_excluir + separacao + field_name;
									}

									$(".arquivoExcluir").attr("type", "text");
									$(".arquivoExcluir").val(campos_para_excluir);
									$(".arquivoExcluir").attr("type", "hidden");

								});
							</script>';

				$input .= '<input type="hidden" name="data['.$modelKey.'][excluir]" id="arquivoExcluir" class="arquivoExcluir" />';


				$input .= $this->hidden($fieldName.'_Control', $options).'<div id="'.$fieldNameControlDiv.'" style="color:red"></div>';

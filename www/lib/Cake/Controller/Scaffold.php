	...
	protected function _scaffoldSave(CakeRequest $request, $action = 'edit') {
		...
		if ($this->controller->beforeScaffold($action)) {
			...
			if (!empty($request->data)) {
				...
				if($action=='edit') {
// 					echo 'Editando...<br>';
// 					print_r($request->data[$modelClass]);
				}
				// echo array_key_exists('deleteimagens', $request->data[$modelClass]);
// 				echo '<hr>';
				// print_r ($request->data[$modelClass]['deleteimagens']);
				// die('<br>aqui');

				if (!empty($request->data[$modelClass]['deleteimagens'])) {
					if(array_key_exists('deleteimagens', $request->data[$modelClass]) && $request->data[$modelClass]['deleteimagens']!='') {
						...
						$registro = $this->$modelClass->find('first', array('conditions' => array($modelClass.'.id' => $this->data[$modelClass]['id'])));
						...
						foreach($delete as $fieldKey) {
							...
							if (file_exists($fieldVal_th) && file_exists($fieldVal)){
								unlink($fieldVal_th);
								unlink($fieldVal);
							}
						}
				...
				if (!empty($request->data[$modelClass]['excluir'])) {
					$campos = explode(",", $request->data[$modelClass]['excluir']);
					$campos_banco = array_values($campos);


					echo "<br><br> Excluindo arquivos dos campos<br>";
					print_r($campos_banco);
					echo "<br>";



					foreach($campos_banco as $fieldKey) {
						$registro = $this->$modelClass->find('first', array(
																		'fields' => $fieldKey,
																		'conditions' => array(
																			$modelClass.'.id' => $this->data[$modelClass]['id']
																		)
																	));

						$fieldVal		= $registro[$modelClass][$fieldKey];
						echo "fieldVal: " . $fieldVal . "<br>";

						$request->data[$modelClass][$fieldKey] = '';

						if (file_exists($fieldVal)){
							unlink($fieldVal);
						}
					}
				}

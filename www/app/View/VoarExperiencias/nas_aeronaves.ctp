<?php
	...
	$this->start('og:title');
        ?>
        <meta property="og:title" content="<?=__d('default', 'Nas Aeronaves')?>"/>
        <?php
    $this->end();
...
?>
<!-- ################################################## VERIFICAR ################################################################### -->
<!--############################### AO COLOCAR O A HREF DO FANCYBOX, A NAVEGAÇÃO PARA DE FUNCIONAR....########################################### -->
...
<div class='row exp_de_voar'>
				...
				<div class='first'>
					...
					<?php if(!empty($fotos[0][$model]['img_'.$lang.'_file'])){ ?>
						<div class='left_'>
							...
							<a href="javascript: void(0);" title="<?php echo $fotos[0][$model]['titulo_'.$lang]?>" onclick="$('a.fancyboxThumb').eq(0).trigger('click'); ">
							...
							<div class='orbit_aeronaves'>
								<div class='elementsHere'>
									...
									foreach($fotos as $foto):
										...
										<div class='elementToSlide' align='center' >
											<a href="<?php echo Router::url('/'.$foto[$model]['img_'.$lang.'_file'], true)?>" rel="galery" class="fancyboxThumb" title="<?=$foto[$model]['titulo_'.$lang]?>" >
												<?=$this->Html->image('/'.$foto[$model]['img_'.$lang.'_file'], array(
	...
	<div class='social_share right margin-bootom20'>
		<a href="<?= Router::url('', true) ?>" share-pj='facebook'><div></div></a>
		<a href="<?=urlencode(__d('default', 'Nas Aeronaves')) ?>+via @abear_br+<?=Router::url('', true)?>" share-pj='twitter'><div></div></a>
		<a href="<?= Router::url('', true) ?>" modal-pj='.modal_mail'><div></div></a>
		<a href="<?= Router::url('', true) ?>" share-pj-pinimg="<?=Router::url('/uploads/img/logo/abear_200px.png', true)?>" share-pj-pindesc="<?=__d('default', 'Nas Aeronaves')?>" share-pj='pinterest' ><div></div></a>
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		// $('a.fancyboxThumb').click(function() { return false; });
	...
	function setInfo(i){
		...
		$('.orgaos .first .left_ > a img').attr('src', obj.attr('src'));
		$('.orgaos .first .left_ > a').attr('onclick', "$('a.fancyboxThumb').eq("+i+").trigger('click');");
		...
		$('.fancybox').attr('title', obj.attr('rtitle'));

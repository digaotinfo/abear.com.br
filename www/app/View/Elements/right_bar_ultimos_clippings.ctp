...
<div class='right_ associa_Rbar'>
	...
	<?php
	if (!empty($lastClippings)) {
		?>
		<div id='mais_lidas'>
			<div class='title'><p><?=__d('default', 'Últimos Clippings')?></p></div>
			
			
			<?php
			foreach ($lastClippings as $registro):
				?>
				<div class='noticia_'>
					<a href='<?=$this->Html->url(array(
													'controller' => 'Imprensas', 
													'action' => 'clipping',
													$registro[$model]["url_amigavel_{$lang}"]
												))?>'>
						<div class='seta mais-lidas'></div>
						<div class='conteudo'><p><i><?=$registro[$model]['titulo_'. $lang]?></i></p></div>
					</a>
				</div>
				<?php
			endforeach;

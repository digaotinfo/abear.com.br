	...
	function beforeFilter()
	{
		...
		///===> FRONT-END
			...
			if ($this->name != 'P28n') {
				...
				/////////////////////////////////////////////====>
				//Redirect controller != PTBR
				/////////////////////////////////////////////////////////////////
				if($lang != 'ptbr'){
					if(
						$this->params['action'] == 'etica'
						|| $this->params['action'] == 'voar_mais_brasil'
						|| $this->params['action'] == 'nas_aeronaves'
						|| $this->params['controller'] == 'eventos'
						|| $this->params['controller'] == 'imprensas'
						|| $this->params['controller'] == 'dadosFatos'
...
//>>> busca admin
	public function buscaGenerica($showThisFields=null){
		if($this->request->is('post')){
			if(!empty($this->request->data['FormBuscaGeneric'])):
				...
				$myRequest = $this->request->data['FormBuscaGeneric']['buscarpor'];
				$conditions = '';




				//>>> montar array dos campos do banco
				$fieldsName = array();
				foreach ($this->$myModel->schema() as $fields => $type):
					if($type['type'] == 'string'){
						$addFields = $fields;
						array_push($fieldsName, $addFields);
					}
				endforeach;
				//<<< montar array dos campos do banco
				...
				//>>> string da conditions
				foreach($fieldsName as $field){
					if($field === end($fieldsName)){
						$conditions .= $myModel.'.'.$field." LIKE '%".$myRequest."%' ";
					}else{
						$conditions .= $myModel.'.'.$field." LIKE '%".$myRequest."%' or  ";
					}
				}
				//<<< string da conditions
				...
				$this->set('showThisFields', $showThisFields);
			endif;
		}
	}
//<<< busca admin

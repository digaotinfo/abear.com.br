$( document ).ready(function() {
	...
	$('.galeria_ .videos_hold a[video-click]').click(function(e){
		...
		obj.attr('src', 'http://img.youtube.com/vi/'+tempEnd.split('/')[4].split('"')[0]+'/0.jpg');
    ...
    /* DROPDOWN MENU */
	    ...
	    //OLD
	    // $('.f-dropdown').parent().mouseenter(function(){
	    // 	console.log('oooasd');
	    // 	var id = $(this).find('ul').attr('id');
	    // 	//setTimeout(function(){ 
	    // 		$('.f-dropdown').hide(0);
	    // 		$('.f-dropdown#'+id).show(0);
	    // 	//}, 100);
	    // });
...
*/
	...
	for (var i = 0; i < $('.passageiros .icone').length; i++) {
		$('.passageiros .icone').eq(i).attr('number', i).bind('click', function(p){
			...
			if(!$(this).hasClass('active')){
				...
				$(this).addClass('active');
				$('.content').hide(100);
				console.log((webroot) + "experiencia-de-voar/guia-de-passageiros/icones/"+ (id));
	...
	/// PAGINATOR ///
		...
		$('.pagina_eventos .hold .arrowL, .pagina_eventos .hold .arrowR').each(function(index, value){
			var obj = $(value);
			if(obj.is('span')){
...
function postToModal(id, adress){
	$.post( (webroot) + adress + '/' + (id), function(data){
		// console.log('endereco> ' + (webroot) + adress + '/' + (id));
		console.log((webroot) + adress + '/' + (id));
		//console.log(data);
